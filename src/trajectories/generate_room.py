...
Python API
----------
>>> from trajectories.generate_room import create_room
...
---
    ...
    uv run python -m trajectories.generate_room --shape square --width 150

    uv run python -m trajectories.generate_room \\
...
def create_room(
    ...
    reference files stored in `data/100x100_square/` and
    `data/200x200_square/`.
    ...
    --------
    >>> create_room(shape='square', width=150)
    # Creates data/150x150_square/

    >>> create_room(shape='circle', width=200, speeds=['med'],
    ...             boundary_avoidance=[True], seed=0)
    # Creates data/200x200_circle/traj_med_ba.npz only
    """
    # ---- Validate arguments ----
    if height is None:
        height = width
