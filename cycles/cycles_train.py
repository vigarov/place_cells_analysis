...
from cycles.cycles_paths import CKPT_DIR, RESULTS_DIR, ROOMS_DIR  # repo root on sys.path
...
@dataclass
class CyclesResult:
    """Outputs of :func:`run_cycles_experiment`."""
...
def _train_room_visit(
    ...
    if config.train_mode == "default":
        ...
        _train_steps(rae, optimizer, traj, wsm, device, config, mask_generator)
    else:
        for i in range(raw_traj_coord.shape[0]):
            traj_i = rebatch_trajectories(raw_traj_coord[i : i + 1], n_segments=8)
            _train_steps(rae, optimizer, traj_i, wsm, device, config, mask_generator)
