...
Usage
-----
    uv run python scripts/generate_oldcycles_rooms.py

    uv run generate-oldcycles-rooms --output data/cycles --seed 0

    # Shorter smoke test
    uv run python scripts/generate_oldcycles_rooms.py --n-rooms 2 --trial-duration-s 60
"""
...
from experiments.old_cycles.cycles_data import trial_steps_from_duration
from trajectories.constants import DEFAULT_PADDING
from trajectories.room_generator import make_square_room, save_arena_map
...
DEFAULT_OUTPUT_DIR = DATA_DIR / "cycles"
DEFAULT_SPEED = DEFAULT_CYCLES_SPEED
DEFAULT_BOUNDARY_AVOIDANCE = DEFAULT_CYCLES_BOUNDARY_AVOIDANCE

# WSM / EV defaults (match analysis/notebook/training_methods.ipynb)
