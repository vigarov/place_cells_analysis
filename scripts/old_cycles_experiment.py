...
`cycles` baseline experiment as in the paper. 
(no warmup, Adam-only, no gradient/optimizer-signal capture)

Marked as "old" because unused anymore.
...
`src/experiments/old_cycles/notebook/cycles_experiment.ipynb` for Supplemental
Figures 1 & 2.
...
Usage::
    ...
    uv run old-cycles-experiment

    uv run old-cycles-experiment --config input_configs/all_cycles.json

    uv run old-cycles-experiment --smoke-test

    uv run old-cycles-experiment --load-only
"""
import argparse
